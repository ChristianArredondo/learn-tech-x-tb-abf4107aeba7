import { IEntityCommerceUserEvent, TCommerceEventType } from '@/types/ecommerce/IEntityCommerceUserEvent';
...
export default class EntityCommerceUserEventService
    extends BaseMongoEntityService<IEntityCommerceUserEvent> {

    public getCollectionName(): MongoCollectionName {
        return MongoCollectionName.COMMERCE_USER_EVENT;
    }

    public async genRecentOrderDetails(): Promise<IModelCommerceUserEventWithReferences[]> {
    ...
    public async genNumRecentEventByType(eventType: TCommerceEventType): Promise<number> {
        const db = await this.genDb();

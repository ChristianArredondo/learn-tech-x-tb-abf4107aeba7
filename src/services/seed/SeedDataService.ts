...
const CONFIG = {
    ...
    randomUserEndpoint: 'https://random-data-api.com/api/v2/users',
    randomProductEndpoint: 'https://random-data-api.com/api/v2/appliances',
};
...
export class SeedDataService {
    ...
    private getEventTotals(): IModelCommerceEventTotals {
        ...
        const numSessions = Math.ceil((Math.random() * sessionsMinMaxDelta))
            + CONFIG.minNumSessions;
    ...
    private async genSeedProducts(): Promise<void> {
        ...
        const productPayloadPromises = Array.from({ length: totalProducts }).map(() => this.genSingleProductPayload());
        const productPayloads = await Promise.all(productPayloadPromises);
        const bulkWritePayloads: AnyBulkWriteOperation<IEntityCommerceProduct>[] = productPayloads
    ...
    private async genSingleProductPayload(): Promise<IEntityCommerceProduct | null> {
        try {
            const randomUserData = await fetch(CONFIG.randomProductEndpoint, { cache: 'no-cache' });
            const { equipment: name } = await randomUserData.json();
    ...
    private async genSeedUsers(): Promise<void> {
        ...
        const userPayloadPromises = Array.from({ length: totalUsers }).map(() => this.genSingleUserPayload());
        const userPayloads = await Promise.all(userPayloadPromises);
        const bulkWritePayloads: AnyBulkWriteOperation<IEntityCommerceUser>[] = userPayloads
    ...
    private async genSingleUserPayload(): Promise<IEntityCommerceUser | null> {
        try {
            const randomUserData = await fetch(CONFIG.randomUserEndpoint, { cache: 'no-cache' });
            const {
                first_name,
                last_name,
                avatar: profile_url,
                address: { street_address, city, state, zip_code, country }
            } = await randomUserData.json();
            return {
                _id: EntityCommerceUserService.getObjectId(),
                first_name,
                last_name,
    ...
    private async genSeedSessionEvents(
        numEvents: number
    ): Promise<void> {
        console.log('---Seeding session events---');

        const sessionPayloadPromises = Array.from({ length: numEvents }).map(() => this.genSingleSessionEventPayload());
        const sessionPayloads = await Promise.all(sessionPayloadPromises);
        const bulkWritePayloads: AnyBulkWriteOperation<IEntityCommerceUserEvent>[] = sessionPayloads
            .filter((sessionEvent): sessionEvent is IEntityCommerceUserEvent => sessionEvent != null)
            .map(sessionEvent => {
                return {
                    insertOne: {
        ...
        const bulkOp = await commerceUserEventService.genBulkWrite(bulkWritePayloads);
        console.log(`
    ...
    private async genSeedProductViewEvents(
        numEvents: number
    ): Promise<void> {
        console.log('---Seeding product-view events---');

        const sessionPayloadPromises = Array.from({ length: numEvents }).map(() => this.genSingleProductViewEventPayload());
        const sessionPayloads = await Promise.all(sessionPayloadPromises);
        const bulkWritePayloads: AnyBulkWriteOperation<IEntityCommerceUserEvent>[] = sessionPayloads
            .filter((sessionEvent): sessionEvent is IEntityCommerceUserEvent => sessionEvent != null)
            .map(sessionEvent => {
    ...
    private async genSeedCheckoutEvents(
        numEvents: number
    ): Promise<void> {
        console.log('---Seeding checkout events---');

        const sessionPayloadPromises = Array.from({ length: numEvents })
            .map(() => this.genSingleCheckoutEventPayload());
        const sessionPayloads = await Promise.all(sessionPayloadPromises);
        const bulkWritePayloads: AnyBulkWriteOperation<IEntityCommerceUserEvent>[] = sessionPayloads
            .filter((sessionEvent): sessionEvent is IEntityCommerceUserEvent => sessionEvent != null)
            .map(sessionEvent => {
    ...
    private async genSeedPurchaseEvents(
        numEvents: number
    ): Promise<void> {
        console.log('---Seeding purchase events---');

        const sessionPayloadPromises = Array.from({ length: numEvents })
            .map(() => this.genSinglePurchaseEventPayload());
        const sessionPayloads = await Promise.all(sessionPayloadPromises);
        const bulkWritePayloads: AnyBulkWriteOperation<IEntityCommerceUserEvent>[] = sessionPayloads
            .filter((sessionEvent): sessionEvent is IEntityCommerceUserEvent => sessionEvent != null)
            .map(sessionEvent => {
